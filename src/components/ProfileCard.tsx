...
import { RefreshCw, CheckCircle, XCircle, Clock, Loader, Route, Dumbbell } from 'lucide-react'
import { WORKOUT_TYPES } from '../types'
import type { Activity, SportFilter, WorkoutType } from '../types'
...
export function ProfileCard({ activities, filter = 'all' }: ProfileCardProps) {
  ...
  const filteredActivities = filter === 'all'
    ? activities
    : isGym
      ? activities.filter(a => WORKOUT_TYPES.includes(a.type as WorkoutType))
      : activities.filter(a => a.type === filter)

  const now = new Date()
  const currentYear = now.getFullYear()
  const currentMonth = now.getMonth()
  const currentYearActivities = filteredActivities.filter((a) => {
    const date = new Date(a.start_date_local)
    return date.getFullYear() === currentYear
  })
  const currentMonthActivities = currentYearActivities.filter(
    (a) => new Date(a.start_date_local).getMonth() === currentMonth
  )
  ...
  const yearlyGymSeconds = currentYearActivities.reduce((s, a) => s + parseMovingTime(a.moving_time), 0)
  const monthlyGymSeconds = currentMonthActivities.reduce((s, a) => s + parseMovingTime(a.moving_time), 0)
  ...
  const latestCandidates = isGym
    ? activities.filter((a) => a.type === 'WeightTraining')
    : filteredActivities
  const latest = latestCandidates.length > 0
    ? [...latestCandidates].sort((a, b) => new Date(b.start_date_local).getTime() - new Date(a.start_date_local).getTime())[0]
    : null
      ...
      <div className="grid grid-cols-3 gap-2 mt-4 text-center border-t border-[var(--color-border)] pt-4">
        <div>
          <p className="text-xs text-[var(--color-muted)] flex items-center justify-center gap-1">
            ...
            {isGym ? (locale === 'zh' ? '今年总次数' : 'This year') : t('activities')}
          </p>
          <p className="text-lg font-bold">{(isGym ? currentYearActivities.length : totalCount).toLocaleString()}</p>
        </div>
        ...
        <div>
          <p className="text-xs text-[var(--color-muted)] flex items-center justify-center gap-1">
            ...
            {isGym ? (locale === 'zh' ? '月度健身时间' : 'Monthly gym time') : (locale === 'zh' ? '时间' : 'Time')}
          </p>
          <p className="text-lg font-bold">{formatHours(isGym ? monthlyGymSeconds : totalSeconds)}</p>
        </div>
      </div>
        ...
        <div className="mt-4 pt-4 border-t border-[var(--color-border)]">
          <div className="flex items-center justify-between mb-1">
            <p className="text-xs text-[var(--color-muted)]">
              {isGym ? (locale === 'zh' ? '最近力量训练' : 'Latest Strength Training') : (locale === 'zh' ? '最近活动' : 'Latest Activity')}
            </p>
            <div className="flex items-center gap-2">
              ...
              {token && (
                <button
                  ...
                  title={locale === 'zh' ? '触发 Garmin 同步' : 'Trigger Garmin sync'}
          ...
          <p className="text-sm font-medium">
            ...
            {latest.name || (latest.type === 'Run' ? 'Run' : latest.type === 'Ride' ? 'Ride' : 'Strength Training')}
            <span className="text-[var(--color-muted)] font-normal">
              {' · '}{isGym ? formatHours(parseMovingTime(latest.moving_time)) : `${formatDistance(latest.distance)} km`} · {formatDate(latest.start_date_local)}
            </span>
          </p>
        </div>
      )}
